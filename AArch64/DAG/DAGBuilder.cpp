...
void AArch64DAGBuilder::visit(const MachineInstr &mi) {
...
/// code properties.
void AArch64DAGBuilder::visitCC(const MachineInstr &mi, MachineSDNode *mnode) {
  ...
  // AArch64::CPSR register use index in MachineInstr.
  int idx = mi.findRegisterUseOperandIdx(AArch64::NZCV);
  // Number of operands for MachineInstr.
  int numOps = mi.getNumOperands();

  // If the MachineInstr has AArch64::CPSR register, update the NodePropertyInfo
  // properties.
  if (idx != -1 && !mi.getOperand(idx).isImplicit()) {
    // MI with AArch64::CPSR register.
    if (idx != numOps - 1) {
