...
AArch64IREmitter::AArch64IREmitter(BasicBlock *bb, AArch64DAGRaisingInfo *dagInfo,
...
#define INVALID_INST (InstructionOpcodesCount + 1)
  ...
  */
  ...
  case ISD::XOR:
    return Xor;
  case EXT_AArch64ISD::LOAD:
    return Load;
  case EXT_AArch64ISD::STORE:
    return Store;
  case ISD::TRUNCATE:
    return Trunc;
...
Value *AArch64IREmitter::getIRValue(SDValue val) {
...
void AArch64IREmitter::emitCondCode(unsigned CondValue, BasicBlock *BB,
                             BasicBlock *IfBB, BasicBlock *ElseBB) {
  switch (CondValue) {
  default:
    break;
  case AArch64CC::EQ: { // EQ  Z set
  ...
  case AArch64CC::LS: { // LS  C clear or Z set
    Value *C_Flag = IRB.CreateLoad(FuncInfo->AllocaMap[2]);
    Value *Z_Flag = IRB.CreateLoad(FuncInfo->AllocaMap[1]);
  ...
  case AArch64CC::LE: { // LE  Z set or N != V
    Value *N_Flag = IRB.CreateLoad(FuncInfo->AllocaMap[0]);
    Value *Z_Flag = IRB.CreateLoad(FuncInfo->AllocaMap[1]);
    Value *V_Flag = IRB.CreateLoad(FuncInfo->AllocaMap[3]);
    Value *InstZEQ = IRB.CreateICmpEQ(Z_Flag, IRB.getTrue());
  ...
  case AArch64CC::AL: { // AL
    assert(false && "Emit conditional code [AArch64CC::AL]. Should not get here!");
  } break;
  }
}

/// Create PHINode for value use selection when running.
PHINode *AArch64IREmitter::createAndEmitPHINode(SDNode *Node, BasicBlock *BB,
                                         BasicBlock *IfBB, BasicBlock *ElseBB,
                                         Instruction *IfInst) {
...
void AArch64IREmitter::emitCPSR(Value *Operand0, Value *Operand1, BasicBlock *BB,
                         unsigned Flag) {
...
void AArch64IREmitter::emitSpecialCPSR(Value *Result, BasicBlock *BB, unsigned Flag) {
  Type *Ty = IRB.getInt1Ty();
  // Update N flag.
  Value *N_Flag = IRB.CreateLShr(Result, IRB.getInt32(31));
...
Type *AArch64IREmitter::getIntTypeByPtr(Type *pty) {
...
void AArch64IREmitter::emitBinaryCPSR(Value *Inst, BasicBlock *BB, unsigned Opcode,
                               SDNode *Node) {
...
void AArch64IREmitter::emitBinary(SDNode *Node) {
...
void AArch64IREmitter::emitSDNode(SDNode *Node) {
  unsigned Opc = Node->getOpcode();
  BasicBlock *BB = getBlock();
...
void AArch64IREmitter::emitSpecialNode(SDNode *Node) {
  unsigned Opc = Node->getOpcode();
  Module &M = *MR->getModule();
  ...
  case EXT_AArch64ISD::BRD: {
    ...
    if (ArgNum > 0) {
      ...
      for (unsigned i = 0; i < ArgNum; i++) {
        if (i < 4)
          ArgVal = FuncInfo->ArgValMap[AArch64::R0 + i];
        else {
