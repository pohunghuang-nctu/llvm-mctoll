//===- AArch64FunctionRaisingInfo.cpp - Binary raiser utility llvm-mctoll --------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the implementaion of AArch64FunctionRaisingInfo class for use
// by llvm-mctoll.
//
//===----------------------------------------------------------------------===//
...
/// Initialize this AArch64FunctionRaisingInfo with the given Function and its
/// associated MachineFunction.
void AArch64FunctionRaisingInfo::set(AArch64ModuleRaiser &mr, Function &fn,
...
SDValue AArch64FunctionRaisingInfo::getValueByRegister(unsigned reg) {
  assert((RegValMap.count(reg) != 0) &&
         "Can not find the corresponding value!");
  return SDValue(RegValMap[reg], 0);
}

void AArch64FunctionRaisingInfo::setValueByRegister(unsigned reg, SDValue val) {
  assert((val.getNode() != nullptr) && "Can not map a nullptr to a register!");
  RegValMap[reg] = val.getNode();
}

SDValue AArch64FunctionRaisingInfo::getValFromRegMap(SDValue val) {
...
/// AArch64FunctionRaisingInfo to an empty state, ready to be used for a
/// different function.
void AArch64FunctionRaisingInfo::clear() {
  MBBMap.clear();
  ValueMap.clear();
  VisitedBBs.clear();
  RegValMap.clear();
...
BasicBlock *AArch64FunctionRaisingInfo::getBasicBlock(MachineBasicBlock &mbb) {
  for (auto bb : MBBMap) {
    if (bb.second == &mbb)
...
BasicBlock *AArch64FunctionRaisingInfo::getOrCreateBasicBlock(MachineBasicBlock *mbb) {
  Function *fn = getCRF();
  if (mbb == nullptr)
